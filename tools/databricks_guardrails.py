...
def check_databricks_notes(findings: list[Finding]) -> None:
    readme = REPO_ROOT / "README.md"
    if not readme.exists():
        return
    text = read_text(readme)
    if "Databricks" not in text and "databricks" not in text:
        add(findings, "WARN", "docs.databricks", "README should mention Databricks and how validations are performed safely.", readme)


def main() -> int:
    parser = argparse.ArgumentParser(description="Offline Databricks governance guardrails for this repo.")
    ...
    check_databricks_notes(findings)
